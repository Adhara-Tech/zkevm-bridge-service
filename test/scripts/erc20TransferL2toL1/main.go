...
const (
  ...
  bridgeURL        = "http://localhost:8080"
	...
	funds              = 133330000000001234 // nolint
...
func main() {
  ...
  log.Infof("ERC20 [L2->L1] Deploy token")
	tokenAddr, err := deployToken(ctx, l2client, l2auth, common.HexToAddress(l2BridgeAddr))
	if err != nil {
  	log.Fatal("Error: ", err)
  }
	log.Infof("ERC20 [L2->L1] L1: Token Addr: ", tokenAddr.Hex())
	...
	tx, err := sendBridgeAsset(ctx, l2client, tokenAddr, amount, destNetwork, &destAddr, []byte{}, l2auth)
	if err != nil {
		log.Fatal("Error: ", err)
	}
  err = operations.WaitTxToBeMined(ctx, l2client, tx, miningTimeout*time.Second)
  if err != nil {
  	log.Fatal("error: ", err)
  }
  ...
  err = manualClaimDepositL1(ctx, l1auth, l1client, bridgeURL, deposit)
  if err != nil {
    log.Fatal("Error: ", err)
  }
	log.Info("Success!")
...
func manualClaimDepositL1(ctx context.Context, auth *bind.TransactOpts, c *utils.Client, bridgeURL string, deposit *pb.Deposit) error {
